...
interface MRTTableProps {
	data: CultureTypes[];
	setOnDelete: (knowledge: CultureTypes) => void; // Update this line
...
const MRTTable: React.FC<MRTTableProps> = ({
	...
	const columns = useMemo<MRT_ColumnDef<CultureTypes>[]>(
		() => [
			{
				accessorKey: "id",
				header: "ID",
...
import { deleteCulture, getAllCulture } from "../../../services/admin/culture";
import { CultureTypes } from "../../../types/culture";

export const AdminCulture: React.FC = () => {
	const [data, setData] = useState<CultureTypes[]>([]);
	const [loading, setLoading] = useState<boolean>(true);
	const [error, setError] = useState<string | null>(null);
	const [onDelete, setOnDelete] = useState<undefined | CultureTypes>(undefined);
	...
	const handleDelete = async (entertainments: CultureTypes) => {
		await deleteCulture(entertainments);
	...
	return (
		<AdminSiteWrapper>
			<Box padding={"12px 0"}>
				<Typography color={"primary"}>Data Kebudayaan</Typography>
			</Box>{" "}
			...
			>
				...
				<DialogActions>
					...
					<Button
						onClick={() => handleDelete(onDelete as CultureTypes)}
						autoFocus
					>
						Agree
					</Button>
