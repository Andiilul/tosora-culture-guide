...
interface MRTTableProps {
	data: LocalKnowledgeTypes[];
	setOnDelete: (knowledge: LocalKnowledgeTypes) => void; // Update this line
...
const MRTTable: React.FC<MRTTableProps> = ({
	...
	const columns = useMemo<MRT_ColumnDef<LocalKnowledgeTypes>[]>(
		() => [
			{
				accessorKey: "id",
				header: "ID",
...
import {
	...
	getAllLocalKnowledge,
} from "../../../services/admin/local-knowledge";
import { LocalKnowledgeTypes } from "../../../types/local-knowledge";

export const AdminLocalKnowledge: React.FC = () => {
	const [data, setData] = useState<LocalKnowledgeTypes[]>([]);
	const [loading, setLoading] = useState<boolean>(true);
	const [error, setError] = useState<string | null>(null);
	const [onDelete, setOnDelete] = useState<undefined | LocalKnowledgeTypes>(
		undefined
	);
	...
	const handleDelete = async (knowledge: LocalKnowledgeTypes) => {
	...
	return (
		<AdminSiteWrapper>
			<Box padding={"12px 0"}>
				<Typography color={"primary"}>Data Pengetahuan Lokal</Typography>
			...
			>
				...
				<DialogContent>
					<DialogContentText id="alert-dialog-description">
						Are You Sure You Want to Delete this Local Knowledge?
					</DialogContentText>
				</DialogContent>
				<DialogActions>
					<Button onClick={() => setOnDelete(undefined)}>Disagree</Button>
					<Button
						onClick={() => handleDelete(onDelete as LocalKnowledgeTypes)}
						autoFocus
					>
						Agree
					</Button>
